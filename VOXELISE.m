function [gridOUTPUT,faces] = VOXELISE(gridX,gridY,gridZ,varargin)
...
if ~isempty(strfind(raydirection,'y'))  &&  (min(gridCOy)>meshYmin || max(gridCOy)<meshYmax)
    gridcheckY = 0;
...
if ~isempty(strfind(raydirection,'z'))  &&  (min(gridCOz)>meshZmin || max(gridCOz)<meshZmax)
    gridcheckZ = 0;
...
faces = uint32( false( voxcountX,voxcountY,voxcountZ,numel(raydirection) ) );
countdirections = 0;

if strfind(raydirection,'x')
    countdirections = countdirections + 1;
    [ g, f ] = VOXELISEinternal(gridCOy,gridCOz,gridCOx,meshXYZ(:,[2,3,1],:));
    gridOUTPUT(:,:,:,countdirections) = permute( g,[3,1,2] );
    faces(:,:,:,countdirections) = permute( f,[3,1,2] );
end

if strfind(raydirection,'y')
    countdirections = countdirections + 1;
    [ g, f ] = VOXELISEinternal(gridCOy,gridCOz,gridCOx,meshXYZ(:,[3,1,2],:));
    gridOUTPUT(:,:,:,countdirections) = permute( g,[2,3,1] );
    faces(:,:,:,countdirections) = permute( f,[2,3,1] );
end

if strfind(raydirection,'z')
    countdirections = countdirections + 1;
    [gridOUTPUT(:,:,:,countdirections),faces(:,:,:,countdirections)] = VOXELISEinternal(gridCOx,gridCOy,gridCOz,meshXYZ);
end
...
%==========================================================================
function [gridOUTPUT,faces] = VOXELISEinternal(gridCOx,gridCOy,gridCOz,meshXYZ)
...
faces = uint32(false(voxcountX,voxcountY,voxcountZ));
            ...
            if isempty(possibleCROSSLIST)==0  %Only continue the analysis if some nearby facets were actually identified
                ...
                if rem(numel(gridCOzCROSS),2)==0  % Only rays which cross an even number of facets are voxelised
                    ...
                    for loopASSIGN = 1:(numel(gridCOzCROSS)/2)
                        ...
                        faces(loopX,loopY,find(voxelsINSIDE,1)) = gridCOzFACE(2*loopASSIGN-1);
                        faces(loopX,loopY,find(voxelsINSIDE,1,'last')) = gridCOzFACE(2*loopASSIGN);
                    end %for
...
if countCORRECTIONLIST>0
    ...
    for loopC = 1:countCORRECTIONLIST
        ...
        f = [ faces(correctionLIST(loopC,1)-1,correctionLIST(loopC,2)-1,:) ,...
            faces(correctionLIST(loopC,1)-1,correctionLIST(loopC,2),:)   ,...
            faces(correctionLIST(loopC,1)-1,correctionLIST(loopC,2)+1,:) ,...
            faces(correctionLIST(loopC,1),correctionLIST(loopC,2)-1,:)   ,...
            faces(correctionLIST(loopC,1),correctionLIST(loopC,2)+1,:)   ,...
            faces(correctionLIST(loopC,1)+1,correctionLIST(loopC,2)-1,:) ,...
            faces(correctionLIST(loopC,1)+1,correctionLIST(loopC,2),:)   ,...
            faces(correctionLIST(loopC,1)+1,correctionLIST(loopC,2)+1,:) ,...
        ...
        f = squeeze( mode( f ) );
        f( isnan( f ) ) = 0;
        faces(correctionLIST(loopC,1),correctionLIST(loopC,2),voxelsforcorrection) = f(voxelsforcorrection);
    end %for
